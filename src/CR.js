...
import { loadPanels, getImagesArray, getScaleX, getScaleY } from './Utils'
...
export default class CR extends Component {
	...
	if(this.props.panelView) {
        ...
        let fullScreenScaleX = getScaleX(page['img_size'][0])
        let fullScreenScaleY = fullScreenScaleX

        //let fullScreenScaleY = getScaleY(page['img_size'][1])
        //let fullScreenScaleX = fullScreenScaleY

        console.log(fullScreenScaleX)
	    ...
	    pages_count: pages.length,
            ...
            offsetX: 0,
            offsetY: 0,
            fullScreenScaleX: fullScreenScaleX,
            fullScreenScaleY: fullScreenScaleY,
        ...
        this.stageTap = this.stageTap.bind(this);
	...
	console.log(e)
	return
	touchPos = this.stage.getPointerPosition()
	if(touchPos.x < window.innerWidth/2) {
		console.log("Touch left")
		this.refreshPage(this.state.page_nr+1)
		return
	}
	...
	if(value == -1) {
		this.refreshComic(this.state.comic_nr-1)
		return
	} else if (value > this.state.pages_count) {
		this.refreshComic(this.state.comic_nr+1)
		return
	}
	...
	let panel_count = 0;
	if(this.state.panelView) {
        	panel_count = parseInt(page['panel_count'])
    ...
    render() {
	    ...
	    onTap={this.stageClick}
            ...
            //onTap={this.onClick}
                ...
                >
     <Group
            scaleX={this.state.hq ? this.state.scaleX * this.state.fullScreenScaleX : this.state.fullScreenScaleX}
            scaleY={this.state.hq ? this.state.scaleY * this.state.fullScreenScaleY: this.state.fullScreenScaleY}
